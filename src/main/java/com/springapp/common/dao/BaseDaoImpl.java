...
public abstract class BaseDaoImpl<T extends BaseEntity> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	public long insert(T t) {
		...
		if (result <= 0)
			throw ApplicationException.DB_INSERT_RESULT_0;
	...
	public long update(T t) {
		...
		if (result <= 0)
			throw ApplicationException.DB_UPDATE_RESULT_0;
	...
	public String getSeqNextValue(String seqName) {
		...
		try {
			...
			throw ApplicationException.DB_GET_SEQ_NEXT_VALUE_ERROR.newInstance("获取序列出现错误!序列名称:{%s}", seqName);
		} finally {
			if (isClosedConn) {
